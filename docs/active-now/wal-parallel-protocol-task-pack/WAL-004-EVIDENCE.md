...
- Final paths are keyed only by the 32-byte complete `WalObjectId`. Candidate
  streams remain invisible until their exact canonical CBOR, EIP-191 signature,
  recovered writer, and domain-separated complete-object ID verify.
- Admission fsyncs the candidate file, atomically renames it, and fsyncs the
  parent. Repeated `put` for an existing ID is idempotent. Reads stream full
  canonical bytes with bounded buffers; ID enumeration is strict byte order.
...
1. Contract tests run against both an independent in-memory implementation and
   `FileWalObjectStore`. Zero-payload, exact configured ceiling, one-range,
   multi-range, out-of-order, agreeing overlap, duplicate, restart, and
   simultaneous-provider cases reconstruct byte-identical objects.
...
pnpm --filter @origintrail-official/dkg-wal test:coverage
  PASS: 18 test files passed, 1 explicit scale file skipped
  PASS: 224 tests passed, 2 explicit scale tests skipped
