...
| `WAL-004` | Implemented and verified | The same branch contains the four-method complete-object store, streaming canonical/signature/ID verification, durable content paths, resumable bounded range staging, crash recovery at every durability point, large-object evidence, adversarial filesystem/quota tests, and 100% package coverage. See `WAL-004-EVIDENCE.md`. |
...
WAL-001, WAL-002, WAL-003, WAL-004, and WAL-005 now share one linear implementation history, one worktree,
