#!/usr/bin/env bash
#
# Experiment B: 4 Claude Code agents in parallel + DKG.
...
echo "=== Experiment B: Multi-Agent + DKG ==="
...
setup_worktree() {
  ...
  if [[ ! -d "$worktree_dir" ]]; then
    ...
    git -C "$OPENCLAW_DIR" worktree add "$worktree_dir" -b "exp-b-${feature_id}" HEAD 2>/dev/null || true
  fi
  echo "$worktree_dir"
}
...
run_feature_bg() {
  ...
  local out_dir="$RESULTS_DIR/round${round}"
  local worktree
  ...
  claude -p \
    ...
    "$prompt" \
    > "$out_dir/${feature_id}.json" 2>&1 || true
}
