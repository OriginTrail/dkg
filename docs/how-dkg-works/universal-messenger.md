...
Automatic retries admit one page at a time, bounded by both entry count and encoded-envelope bytes. The default page holds at most 100 entries and 10 MiB of encoded payloads (`DEFAULT_OUTBOX_DRAIN_MAX_PAYLOAD_BYTES`, the default transport read limit), with four retry workers. SQLite selects metadata first and loads the exact admitted composite keys, in admission order, in one query on the same transaction snapshot. Selection uses three statements regardless of the number of admitted rows. The default accommodates a maximum-size 4 MiB SWM application payload plus its reliable envelope. The byte limit covers retained envelope payloads; it does not include database caches, transport buffers, or the process's other memory.
...
Messenger and `DKGAgentConfig` require `BoundedProtocolOutboxStore`, including the boolean `hasPendingFor` method and all five bounded capabilities: `readDuePage`, `listMetadata`, `dropExpiredMetadata`, `recordRetryFailure`, and `queueStats`. Enforce the count and byte budgets inside storage before materializing payloads. A legacy store lacking these methods is rejected when Messenger is constructed; automatic retries never fall back to unbounded `due()` or `list()` reads. Core’s `BoundedProtocolOutbox` validates this complete contract at construction. Common persistence, bounded access, and legacy inspection are separate capabilities; new bounded stores do not need `due`, `dropExpired`, `list`, or `getEntry`. `ProtocolOutbox` retains the legacy inspection facade. The built-in memory and SQLite stores implement both capabilities. A custom bounded-only store supports automatic retries and metadata diagnostics; explicit legacy payload inspection reports unsupported unless that store also supplies `list` and `getEntry`.
