...
A short snapshot prefix remains missing and is retried from offset zero on a later round, while other references can advance. An incomplete stream or hard failure stops new dispatches and joins all admitted work. Reporting follows manifest order even when downloads finish out of order, including successes that finish after another reference fails. Missing-reference samples stay capped at ten entries.
