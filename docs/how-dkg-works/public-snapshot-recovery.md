...
The requester checks the shared work admission (the recovery job window composed with the round deadline) before starting each reference and again after an asynchronous cache miss, immediately before network dispatch. A prepared walk marks the references whose manifest-bound evidence an owner already validated; those count as ready without cache work or dispatch. A private recovery job keeps one operation in flight, so its unresolved-first priority under a single shared allowance holds; the public and direct walks use the full pool. Already admitted work settles before the round returns, including materialization callbacks, so a round can extend past its admission deadline. Local admission yields, including a page fetch that yielded on the same allowance, do not count as peer timeouts; an incomplete phase caused solely by local admission is attributed as such rather than to the peer.

A short snapshot prefix remains missing and is retried from offset zero on a later round, while other references can advance. An incomplete stream or hard failure stops new dispatches and joins all admitted work. Reporting follows manifest order even when downloads finish out of order, including successes that finish after another reference fails. Missing-reference samples stay capped at ten entries. The requester accounts for all settled bytes, resumptions, completions, timeouts and local yields before rethrowing a fatal result. A timed-out sibling therefore retains its backoff signal even when another operation fails locally. The first fatal failure remains the primary cause; any other admitted fatal failures are retained alongside it. Downstream classifiers share one view of nested causes, including tags added to a group after construction. A mixed failure is eligible for transport-prefix recovery only when every cause is a retryable transport interruption and no group carries conflicting response evidence.
