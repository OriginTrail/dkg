...
The daemon records who owns the store in `dkg-oxigraph-owner.json` in the store directory. It writes the record right after starting Oxigraph (the daemon, the process it launched, and the binary), then adds Oxigraph itself once the store is ready. Each process is identified by PID and process start time. Before each Oxigraph start, the daemon stops a process that holds `<location>/LOCK` only in these cases:
