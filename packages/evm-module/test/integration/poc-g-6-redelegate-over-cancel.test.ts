 ...
 * redelegate break.
 */
describe('@integration PoC G-6: redelegate over-cancels reward-compounded boost', () => {
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // EXPLOIT — DoS: a reward-compounded boosted position CANNOT redelegate.
  // The slot accumulated as floor(R0*2.5)+floor(rew*2.5) (two independent
  // floors) is 1 wei short of the one-shot floor((R0+rew)*2.5) computed by
  // updateOnRedelegate, so _cancelNodeExpiry's require(existing >= drop)
  // reverts. The honest delegator who merely claimed a reward before moving
  // nodes is bricked for the rest of the lock.
  // ──────────────────────────────────────────────────────────────────────────
  it('EXPLOIT: reward-compounded redelegate reverts (one-shot over-unwind) — recoverable-but-long DoS', async () => {
    const { identityId: nodeA } = await createProfile();
    const { identityId: nodeB } = await createProfile();
    ...
    // ── ROOT-CAUSE WITNESS (boost slot) — the boost drop slot is the SUM of
    //    two independent floors and is exactly 1 wei short of the one-shot
    //    boost updateOnRedelegate's _cancelNodeExpiry would compute. ──
    const slotAfterClaim = await CSS.getNodeExpiryDrop(nodeA, expiryTs);
    const accumulatedBoost = boostOf(R0) + boostOf(expectedReward); // two floors
    const oneShotBoost = boostOf(R0 + expectedReward); // one floor (what _cancelNodeExpiry uses)
    expect(slotAfterClaim).to.equal(accumulatedBoost);
    expect(oneShotBoost).to.equal(accumulatedBoost + 1n); // over-cancel: 1 wei > scheduled
    expect(slotAfterClaim).to.be.lessThan(oneShotBoost);

    // ── ROOT-CAUSE WITNESS (running effective stake) — node A's running
    //    effective stake was ALSO built as two independent floors
    //    floor(R0*3.5)+floor(rew*3.5); updateOnRedelegate unwinds it with the
    //    one-shot floor((R0+rew)*3.5), which is again 1 wei MORE than the
    //    accumulated running stake. Because _applyNodeStakeDelta(old,-eff)
    //    runs BEFORE _cancelNodeExpiry, the effective-stake leg is the first
    //    require to trip ("Neg running stake", CSS :947) — but it is the very
    //    same incremental-vs-one-shot floor asymmetry as the boost leg. ──
    const runningA = await CSS.getNodeRunningEffectiveStake(nodeA);
    const accumulatedEff = effOf(R0) + effOf(expectedReward); // two floors
    const oneShotEff = effOf(R0 + expectedReward); // one floor (what the unwind uses)
    expect(runningA).to.equal(accumulatedEff);
    expect(oneShotEff).to.equal(accumulatedEff + 1n); // over-subtract: 1 wei > running
    expect(runningA).to.be.lessThan(oneShotEff);

    // ── THE DoS — redelegate reverts: the one-shot unwind exceeds the
    //    incrementally-accumulated state. Whichever named require trips first,
    //    the core flow is unavailable. Here the effective-stake leg reverts. ──
    await expect(NFT.connect(accounts[0]).redelegate(1, nodeB)).to.be.revertedWith(
      'Neg running stake',
    );

    // Position is bricked on node A: still owned by the delegator, still on A,
    // and B never received any contribution. The core redelegate flow is
    // unavailable until the lock expires.
    expect(await NFT.ownerOf(1)).to.equal(accounts[0].address);
    expect((await CSS.getPosition(1)).identityId).to.equal(BigInt(nodeA));
    expect(await CSS.getNodeRunningEffectiveStake(nodeB)).to.equal(0n);
  });
});
