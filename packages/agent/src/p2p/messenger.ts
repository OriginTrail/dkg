 ...
 *
 * See `docs/architecture/universal-messenger.md` for the architecture overview and per-
  ...
  register(protocolId: string, handler: ReliableHandler): void {
    ...
    this.router.register(protocolId, async (data, peerIdObj) => {
      ...
      try {
        ...
        // design decision — see docs/architecture/universal-messenger.md.
  ...
  constructor(method: string) {
    super(
      `[Messenger] ${method} requires idempotencyStore + outboxStore, ` +
        ...
        `DashboardDB; see docs/architecture/universal-messenger.md).`,
    );
