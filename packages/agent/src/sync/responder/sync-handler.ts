...
interface RegisterSyncHandlerParams {
  ...
  syncPageSize: number;
  /** Explicit live provider, read for each workspace request. */
  getSharedMemoryTtlMs?: () => number;
  /** @deprecated Static test/compatibility setting; prefer getSharedMemoryTtlMs. */
...
export function registerSyncHandler(params: RegisterSyncHandlerParams): void {
  ...
  register(protocolSync, async (data, peerId, options) => withSpan('sync.response', async (span) => {
    ...
    const serveAuthorizedStage = async (authDurationMs: number): Promise<Uint8Array> => {
      ...
      if (isWorkspace) {
        const sharedMemoryTtlMs = params.getSharedMemoryTtlMs?.() ?? params.sharedMemoryTtlMs ?? 0;
