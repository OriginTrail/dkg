...
export type SharedMemorySyncMode = Readonly<
  ...
  | {
    ...
    /** @deprecated Put metadataFetcher on SharedMemorySyncContext. If both are supplied they must be identical. */
...
export async function runSharedMemorySync(context: SharedMemorySyncContext): Promise<SharedMemorySyncSummary> {
  ...
  const metadataFetcher = normalizeSharedMemoryMetadataFetcher(context, request => fetchRecoveryPages(
    request.contextGraphId, 'meta', request.graphUri, request.deadline,
  ));
