...
export class PublishMethods extends DKGAgentBase {
  ...
  async publishAsync(this: DKGAgent,
    ...
    const intent = await this.resolveFinalizedAssertionVmPublishIntent(
      ...
      assertionName,
      {
        authorSelection: { mode: 'author', agentAddress: lifecycleAgentAddress },
        subGraphName: opts?.subGraphName,
  ...
  /** Resolve an authoritative author, caller hint, or resident selection.
   * Default selection uses the node identity as its hint; resident selection
   * preserves the caller while requiring the chosen author to exist locally.
   */
  ...
  async resolveFinalizedAssertionVmPublishIntent(this: DKGAgent,
    contextGraphId: string,
    name: string,
    opts?: PublishAuthorSelectionOptions & {
      subGraphName?: string;
  ...
  /** Re-check the named lifecycle independently of the mutable workspace head. */
  ...
  async _finalizeRecoveredQueuedKnowledgeAssetVmPublish(
    ...
    // A superseded transaction proves only its own durable receipt. Never let
    // that old queue item mutate the current assertion or access metadata.
    if (recovered.materialization.superseded) {
