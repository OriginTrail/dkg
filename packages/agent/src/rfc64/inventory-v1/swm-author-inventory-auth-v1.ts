...
export interface VerifiedMergeSwmAuthorInventoryCommitInputV1 {
  readonly snapshot: SwmAuthorInventorySnapshotV1;
...
/** Observe, canonicalize, and authenticate one exact non-empty merge CAS input. */
export function prepareVerifiedMergeSwmAuthorInventoryCommitInputV1(
  input: CompareAndSwapMergeSwmAuthorInventoryInputV1,
  error: SwmAuthorInventoryErrorFactoryV1,
): VerifiedMergeSwmAuthorInventoryCommitInputV1 {
  try {
    const candidateRecord = snapshotPlainDataRecordV1(
      input,
      'SWM author inventory exact merge CAS input',
    );
    const hasIssuerSignature = Object.keys(candidateRecord).includes('issuerSignature');
    assertExactFieldSetV1(
      candidateRecord,
      hasIssuerSignature
        ? ['snapshot', 'mergeRows', 'issuerSignature', 'expectedCurrentHeadDigest']
        : ['snapshot', 'mergeRows', 'expectedCurrentHeadDigest'],
      'SWM author inventory exact merge CAS input',
    );
    const candidate = candidateRecord as unknown as Readonly<
    ...
    const mergeRows = parseCanonicalSwmAuthorInventoryRowsV1(
      canonicalizeSwmAuthorInventoryRowsBytesV1(candidate.mergeRows),
    );
    if (mergeRows.length === 0) {
      throw new Error('SWM author inventory exact merge requires at least one row');
    }
    const snapshot = Object.freeze({ head, rows });
    assertSwmAuthorInventorySnapshotBindingV1(snapshot);
    ...
    throw error(
      'swm-inventory-input',
      'SWM author inventory exact merge CAS input is not canonical, authenticated, or internally bound',
      { cause },
    );
  }
}
