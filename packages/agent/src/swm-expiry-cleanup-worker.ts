...
export class SwmExpiryCleanupWorker {
  ...
  constructor(
    ...
    private readonly settings: { sharedMemoryTtlMs: number },
    private readonly intervalMs = SWM_CLEANUP_INTERVAL_MS,
  ) { validateSharedMemoryTtlMs(settings.sharedMemoryTtlMs); }

  get running(): boolean {
    return this.settings.sharedMemoryTtlMs > 0 && (this.state.kind === 'scheduled'
  ...
  setTtl(ttlMs: number): void {
    ...
    this.settings.sharedMemoryTtlMs = ttlMs;
    if (ttlMs === 0) this.cancelScheduled();
    else this.schedule(0);
  }

  /** Join one owned flight; a manual request upgrades a periodic flight to a full drain. */
  runNow(): Promise<number> {
    if (this.state.kind === 'stopping' || this.state.kind === 'stopped' || this.settings.sharedMemoryTtlMs === 0) return Promise.resolve(0);
    if (this.state.kind === 'running') {
      const flight = this.state.flight;
      if (flight.request.kind === 'periodic') flight.request = { kind: 'manual', cutoffMs: Date.now() - this.settings.sharedMemoryTtlMs };
      return flight.completion;
    }
    this.cancelScheduled();
    return this.launch({ kind: 'manual', cutoffMs: Date.now() - this.settings.sharedMemoryTtlMs });
  }
  ...
  private async execute(flight: CleanupFlight, pending?: SwmExpiryCleanupContinuation): Promise<number> {
    let deleted = 0;
    let continuation = flight.request.kind === 'manual' ? undefined : pending;
    try {
      while (this.owns(flight) && this.settings.sharedMemoryTtlMs > 0) {
        const request = flight.request;
        const result = await this.processPass(this.settings.sharedMemoryTtlMs, () => !this.owns(flight), continuation,
          request.kind === 'manual' ? request.cutoffMs : undefined);
        deleted += result.triplesDeleted;
        if (!this.owns(flight) || this.settings.sharedMemoryTtlMs === 0) break;
        if (flight.request.kind === 'periodic') {
          flight.continuation = result.continuation;
          break;
        }
        // A manual request joining a periodic pass starts its own finite sweep
        // with its newer cutoff, even if the joined pass exhausted its targets.
        const promoted = request !== flight.request;
        continuation = promoted ? undefined : result.continuation;
        if (!promoted && !continuation) break;
        await setImmediate();
      }
      return deleted;
    } finally {
  ...
  private schedule(delayMs: number, continuation?: SwmExpiryCleanupContinuation): void {
    if (this.state.kind !== 'idle' || this.state.mode !== 'periodic' || this.settings.sharedMemoryTtlMs === 0) return;
