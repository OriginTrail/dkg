...
export class LifecycleSyncMethods extends DKGAgentBase {
  /**
   * Resolve the numeric CG carried by a Random Sampling challenge without
   * requiring subscription rehydration to have populated the reverse map.
   *
   * The fallback never guesses from a numeric id. It reads the immutable name
   * commitment for the challenged slot, matches that commitment against local
   * cleartext graph names, proves that the slot is active, and checks its live
   * access policy. A public graph may be recovered from local storage alone; a
   * private graph additionally requires an active local subscription.
   */
  async resolveRandomSamplingLocalContextGraphId(
    ...
    const now = Date.now();
    const cached = this.randomSamplingContextGraphResolutionCache.get(cacheKey);
    if (cached && cached.expiresAt > now) {
      if (
        !cached.localContextGraphId
        || !cached.requiresSubscription
        || this.subscribedContextGraphs.get(cached.localContextGraphId)?.subscribed === true
      ) return cached.localContextGraphId;
    }
    if (cached) this.randomSamplingContextGraphResolutionCache.delete(cacheKey);

    const rememberMiss = (): undefined => {
      this.randomSamplingContextGraphResolutionCache.set(cacheKey, {
    ...
    const committedNameHash = await getNameHash.call(
      this.chain,
    ...
    const matchesCommitment = (localContextGraphId: string): boolean => {
      const subscription = this.subscribedContextGraphs.get(localContextGraphId);
      if (subscription?.onChainHash?.toLowerCase() === normalizedCommitment) return true;
      try {
        return this.contextGraphNameCommitment(localContextGraphId).toLowerCase()
    ...
    // Chain discovery persists this reverse binding before it mutates the
    // process-local subscription catalogue. Prefer the indexed point lookup:
    // it survives restarts, handles curator/name ids containing '/', and
    // avoids a store-wide graph enumeration on the normal recovery path.
    const ontologyGraph = contextGraphDataGraphUri(SYSTEM_CONTEXT_GRAPHS.ONTOLOGY);
    const onChainIdPredicate = `${DKG_ONTOLOGY.DKG_CONTEXT_GRAPH}OnChainId`;
    try {
      const durableBindings = await this.store.query(`
      ...
      // Older local stores may not carry the durable binding. Continue into
      // the compatibility graph-catalog fallback below.
    }
    let matching = [...candidates].filter(matchesCommitment);
    if (matching.length === 0) {
      let storedContextGraphIds: string[];
      try {
        storedContextGraphIds = await new GraphManager(this.store).listContextGraphs({
          signal,
    ...
    const [active, accessPolicy] = await Promise.all([
      isActive.call(this.chain, onChainContextGraphId),
      getAccessPolicy.call(this.chain, onChainContextGraphId),
    ]);
    ...
    this.randomSamplingContextGraphResolutionCache.set(cacheKey, {
      localContextGraphId,
      requiresSubscription: accessPolicy === 1,
  ...
  repairRandomSamplingKnowledgeAsset(
    ...
    const dependencies = {
      ...
      resolveLocalContextGraphId: async (cgId, signal) => {
        const direct = this.resolveLocalCgIdByOnChainId(cgId);
        if (direct) return direct;
        const fallback = (this as Partial<DKGAgent>).resolveRandomSamplingLocalContextGraphId;
        return typeof fallback === 'function'
          ? fallback.call(this, cgId, signal)
          : undefined;
      },
