...
import { listStoredContextGraphUris, storedSharedMemoryFamilies } from './stored-context-graph-candidates.js';
...
import { PrivateContentStore, createTripleStore, asChangelogReader, type ChangelogReader, type TripleStore, type TripleStoreConfig, type Quad, type LargeLiteralStorageConfig } from '@origintrail-official/dkg-storage';
...
export class LifecycleSyncMethods extends DKGAgentBase {
  ...
  async cleanupExpiredSharedMemory(this: DKGAgent): Promise<number> {
    ...
    try {
      const storedGraphs = await listStoredContextGraphUris(this.store, { source: 'agent.swmCleanup.graphFamilies' });
      // Ownership cache keys explicitly identify the root/subgraph partition.
      // Index those known keys by physical bucket instead of guessing identities
      // from slash-shaped graph names after their declarations have disappeared.
      const ownershipByBucket = new Map<string, string[]>();
      for (const key of this.workspaceOwnedEntities.keys()) {
        const separator = key.indexOf('\0');
        const contextGraphId = separator < 0 ? key : key.slice(0, separator);
        const subGraphName = separator < 0 ? undefined : key.slice(separator + 1);
        const bucket = contextGraphSharedMemoryUri(contextGraphId, subGraphName);
        const keys = ownershipByBucket.get(bucket) ?? [];
        keys.push(key);
        ownershipByBucket.set(bucket, keys);
      }
      for (const { dataGraph: wsGraph, metaGraph: wsMetaGraph } of storedSharedMemoryFamilies(storedGraphs)) {
        let graphDeleted = 0;
        ...
        for (const row of expiredOps.bindings) {
          ...
          for (const ownershipKey of ownershipByBucket.get(wsGraph) ?? []) {
        ...
        if (expiredOpsCount > 0) {
          this.log.info(ctx, `SWM cleanup for "${wsMetaGraph}": evicted ${expiredOpsCount} expired operation(s), ${graphDeleted} triples`);
        }
      }
    } catch (err) {
