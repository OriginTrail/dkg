...
describe('rootless graph-scoped KA lifecycle', () => {
  ...
  it('repairs a named lifecycle after its async publish transaction confirmed during downtime', async () => {
    ...
    // cannot make the row flaky.
    expect(recoveryCleanup.mock.calls.filter((call) => (
      call[3]?.operationName === 'publishFromSWM'
    ))).toHaveLength(0);
...
describe('Query views', () => {
  ...
  it('threads full-content pricing through real sync and queued agent publication boundaries', async () => {
    const agent = await createAgent('FullContentPricingBoundaryBot');
    await agent.createContextGraph({ id: CG_ID, name: 'Full-content pricing boundary' });
    await agent.registerContextGraph(CG_ID);
    ...
    const baseAckProvider = (agent as any).createV10ACKProvider(CG_ID);
    ...
    const stage = async (name: string, suffix: string) => {
      ...
      await agent.assertion.create(CG_ID, name);
      await agent.assertion.write(CG_ID, name, [publicQuad]);
      ...
      const promoted = await agent.assertion.promote(CG_ID, name);
      expect(promoted.publishReady).toBe(true);
      const intent = await agent.resolveFinalizedAssertionVmPublishIntent(CG_ID, name, {
      ...
      const expectedPricingByteSize = resolvePublicationPricingByteSize({
        policy: 'full-content',
      ...
      return { intent, expectedPricingByteSize };
    };

    const assertCapturedPricing = async (expectedPricingByteSize: bigint) => {
      ...
      const expectedTokenAmount = await chain.getRequiredPublishTokenAmount(
        expectedPricingByteSize,
        ack.epochs,
      );
      expect(plan.effectiveByteSize).toBe(expectedPricingByteSize);
      expect(ack.publicByteSize).toBeLessThan(expectedPricingByteSize);
    ...
    try {
      const synchronous = await stage('full-content-sync', 'full-content-sync');
      ...
      await assertCapturedPricing(synchronous.expectedPricingByteSize);

      planSpy.mockClear();
      createSpy.mockClear();
      ackInputs.length = 0;

      const queued = await stage('full-content-queued', 'full-content-queued');
      ...
      ).getKnowledgeAssetPrivateTriples(CG_ID, queuedScope);
      ...
      await assertCapturedPricing(queued.expectedPricingByteSize);
    } finally {
      planSpy.mockRestore();
  ...
  it('rejects full-content pricing on every real update boundary before side effects', async () => {
    ...
    await agent.createContextGraph({ id: CG_ID, name: 'Full-content update guard' });
    await agent.registerContextGraph(CG_ID);

    const name = 'full-content-update-rejected';
    const root = `${ENTITY_BASE}:full-content-update-rejected`;
    await agent.assertion.create(CG_ID, name);
    await agent.assertion.write(CG_ID, name, [{
      subject: root,
      predicate: 'http://schema.org/name',
    ...
    await agent.assertion.promote(CG_ID, name);
    expect((await agent.publishFromFinalizedAssertion(CG_ID, name)).status).toBe('confirmed');
    ...
    const vmBefore = (await agent.assertion.history(CG_ID, name))?.vmCurrentAssertion;
    const expectedError = {
    ...
    try {
      await expect(agent.resolveFinalizedAssertionVmPublishIntent(CG_ID, name, {
        pricingPolicy: 'full-content',
      })).rejects.toMatchObject(expectedError);

      await expect(agent.publishFromFinalizedAssertion(CG_ID, name, {
        pricingPolicy: 'full-content',
      })).rejects.toMatchObject(expectedError);

      const validIntent = await agent.resolveFinalizedAssertionVmPublishIntent(CG_ID, name);
      const publicSnapshot = await resolveKnowledgeAssetOperationPublicQuads({
        store: agent.store,
        graphManager: new GraphManager(agent.store),
        contextGraphId: CG_ID,
      ...
      expect((await agent.assertion.history(CG_ID, name))?.vmCurrentAssertion).toBe(vmBefore);
    } finally {
      planSpy.mockRestore();
