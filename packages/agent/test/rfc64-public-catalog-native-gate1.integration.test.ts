...
describe('RFC-64 Gate 1 native successor to public SWM', () => {
  it('fetches head to row to bundle and activates one exact KA on a live receiver', async () => {
    ...
    const rowBundle = buildRowBundle();
    ...
    const evidence = await receiver.synchronizeOnePublicOpenRow(
      authorNode.peerId,
    ...
    const expectedRowDigest = computeAuthorCatalogRowDigestV1(
      computeAuthorCatalogScopeDigestV1(deriveAuthorCatalogScopeFromHeadV1(successor.head.payload)),
      rowBundle.row,
    );
    expect(evidence).toEqual({
      inventoryDigest: successor.head.objectDigest,
      catalogRowDigest: expectedRowDigest,
      contentDigest: rowBundle.row.projectionDigest,
      bundleDigest: rowBundle.row.transfer.blobDigest,
      kaUal: UAL,
      inventoryRowCount: 1,
    ...
    const activated = await receiverStore.query(
    ...
    expect(authorObjectRead.mock.calls.map(([digest]) => digest)).toEqual([
      successor.head.payload.directoryRootDigest,
      successor.bucket?.objectDigest,
    ]);
    expect(authorBundleRead).toHaveBeenCalledOnce();
    expect(authorBundleRead).toHaveBeenCalledWith(rowBundle.row.transfer.blobDigest);
  }, 30_000);
});

function buildRowBundle(): { row: AuthorCatalogRowV1; bundleBytes: Uint8Array } {
  const seal = {
