...
describe('Phase D - VM reconcile damping', () => {
  ...
  it('keeps slow exact recovery within the per-pass peer budget after cooldown expiry', async () => {
    ...
    try {
      ...
      const targets = peers.map((_, ordinal) =>
        vmRecoveryTarget(localCgId, ordinal, `slow-${ordinal}`));
      ...
      expect(fetches).toHaveLength(DKGAgentBase.VM_RECONCILE_EXACT_PEER_MAX);
      expect(result.attemptedOrdinals).toEqual(Array.from(
        { length: DKGAgentBase.VM_RECONCILE_EXACT_PEER_MAX },
        (_, ordinal) => ordinal,
      ));
      expect(result.continuationOrdinal).toBe(DKGAgentBase.VM_RECONCILE_EXACT_PEER_MAX);
    } finally {
      vi.useRealTimers();
    }
  });
