...
describe('Phase C sync envelope — sinceBatchId is unsigned', () => {
  it('does not feed sinceBatchId into the signed digest', async () => {
    ...
    // requestId, issuedAtMs, agentAddress, authPurpose, authSelector) — 11 args,
    // NO sinceBatchId. requestId (idx 6) and issuedAtMs (idx 7) are random/time-
    // based per build, so compare only the stable, semantically-meaningful positions.
