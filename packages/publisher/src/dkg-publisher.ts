...
import { DEFAULT_PUBLISH_EPOCHS, MAX_PUBLISH_EPOCHS, type Publisher, type BasePublicationOptions, type PublishOptions, type UpdateOptions, type PublishLifecycleHooks, type PublishResult, type KAManifestEntry, type PhaseCallback, type V10CoreNodeACK, type V10ACKProviderParams, type V10ACKProviderObject, type LegacyV10ACKProvider } from './publisher.js';
import {
...
export type PublisherAssertionPromoteOptions = {
  ...
  /** Promote-scoped authority resolver; defaults to the configured workspace resolver. */
  resolveWorkspaceRecipients?: WorkspaceAgentRecipientResolver;
};
...
function resolveGraphScopedPublishDescriptor(
  options: BasePublicationOptions,
...
export class DKGPublisher implements Publisher {
  ...
  private async encodeWorkspaceGossipPayload(
    ...
    resolveRecipients = this.workspaceAgentRecipientResolver,
  ): Promise<EncodedWorkspaceGossipPayload> {
    if (options.localOnly || !resolveRecipients) {
  ...
  private async assertionPromoteUnlocked(
    ...
    if (operationPublisherPeerId) {
      ...
      const wrapped = await tagPromoteStep('encodeWorkspaceGossipPayload', () =>
        this.encodeWorkspaceGossipPayload(
          ...
          opts?.resolveWorkspaceRecipients,
        ),
      );
