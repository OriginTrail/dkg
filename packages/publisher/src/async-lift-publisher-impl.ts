...
export class TripleStoreAsyncLiftPublisher
  ...
  constructor(
    ...
    this.publishAuthority = config.publishAuthorityResolver
      ? new PublishAuthorityCache({
        // Resolve the id the SAME way the publish path does, in the SAME order.
        // `DKGPublisher` uses `BigInt(onChainContextGraphId ?? contextGraphId)` — the stored
        // mapping first, then the name. Probing the name first asked about a different graph
        // than publish would target (a graph named "12" stamped to id 900 was probed as CG 12);
        // probing ONLY the store was worse in the other direction, reporting `unenforced` for a
        // CG named by its numeric id with no local stamp while publish still targeted that id —
        // and with the refusal now terminal, that is permanent job loss rather than a retry.
  ...
  private async readJobPayload(jobId: string): Promise<LiftJobPayloadDecodeResult> {
    ...
    return decodeLiftJobPayload(rows[0]?.['payload']);
  }
