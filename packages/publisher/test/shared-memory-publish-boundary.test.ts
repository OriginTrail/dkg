...
describe('publishFromSharedMemory selection boundary', () => {
  ...
  it('rejects a single-root subset of a multi-root share-operation boundary', async () => {
    const { publisher, store, publishSpy } = await makePublisher();
    const roots = ['urn:test:root:one', 'urn:test:root:two'];
    await store.insert([
      q(roots[0]),
  ...
  it('rejects multi-root selection when a later share updates only one selected root', async () => {
    const { publisher, store, publishSpy } = await makePublisher();
    const roots = ['urn:test:root:one', 'urn:test:root:two'];
    await store.insert([
      q(roots[0]),
      ...
      ...generateShareMetadata({
        contextGraphId: CONTEXT_GRAPH,
        shareOperationId: 'op-original-ab',
        rootEntities: roots,
        publisherPeerId: 'peer-a',
        timestamp: new Date('2026-01-01T00:00:00.000Z'),
      }, SWM_META_GRAPH),
