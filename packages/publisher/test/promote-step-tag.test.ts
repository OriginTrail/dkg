...
describe('#2314 canonical promote stage normalization', () => {
  it('recognizes every canonical producer stage', () => {
    for (const stage of PROMOTE_STAGES) {
      expect(normalizePromoteStage(`[promote:${stage}] failure`)).toEqual({
        stage,
        message: 'failure',
      });
    }
  ...
  it('normalizes unknown and malformed tags without exposing their labels', () => {
    expect(normalizePromoteStage('[promote:caller-secret] failure')).toEqual({
      stage: 'unknown',
      message: 'failure',
    });
    expect(normalizePromoteStage('[promote:unterminated failure')).toEqual({
      stage: 'unknown',
