...
export interface MemoryToolContext {
  ...
  createParanet: (opts: { id: string; name: string; description?: string }) => Promise<void>;
...
export class ChatMemoryManager {
  ...
  async storeChatExchange(
    ...
    const quads: Array<{ subject: string; predicate: string; object: string; graph: string }> = [
      ...
      { subject: userMsgUri, predicate: `${SCHEMA}dateCreated`, object: `"${now}"^^<${XSD_DATETIME}>`, graph: '' },
      ...
      { subject: assistantMsgUri, predicate: `${SCHEMA}dateCreated`, object: `"${now}"^^<${XSD_DATETIME}>`, graph: '' },
    ...
    await this.tools.writeToWorkspace(MEMORY_PARANET, quads);
  }
