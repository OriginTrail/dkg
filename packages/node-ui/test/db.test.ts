...
describe('DashboardDB — selected-only VM reconcile cursors', () => {
  it('keeps progress independent per local CG and numeric chain binding across reopen', () => {
    ...
    expect(db.getSelectedVmReconcileCursor('selected-public', '298')).toMatchObject({
      on_chain_context_graph_id: '298',
      watermark: 7,
    });
    expect(db.getSelectedVmReconcileCursor('selected-public', '299')).toMatchObject({
      on_chain_context_graph_id: '299',
      watermark: 2,
    });
  ...
  it('creates the dedicated table when upgrading a version-31 database', () => {
    db.db.exec('DROP TABLE selected_vm_reconcile_cursors;');
    db.db.pragma('user_version = 31');
    db.close();

    db = new DashboardDB({ dataDir: dir });
    expect(db.db.pragma('user_version', { simple: true })).toBe(32);
    db.upsertSelectedVmReconcileCursor({
      context_graph_id: 'migrated-selected',
      on_chain_context_graph_id: '300',
      name_hash: `0x${'22'.repeat(32)}`,
      watermark: 4,
      updated_at: 300,
    });
    expect(db.getSelectedVmReconcileCursor('migrated-selected', '300')).toMatchObject({
      watermark: 4,
    });
  });
});
