...
const EV = join(homedir(), "odysseus-dkg-proto/nsm-v35-evidence/phase5");
...
try {
  // ═══ RESUME (tab funded in the first pass: J1–J4 green, deposit
  // 0xa679632633db9171be71db5a72298dd9684b3348da83b98b50d440c09564dfbe,
  // tab_e949d1927c2b2231 opened over the lane) — continue at discovery ═══
  ...
  if (false) {
  // (first-pass onboarding steps, already green)
  await page.goto(`http://localhost:${PORT}/ui/?marketplace=1`, { waitUntil: "domcontentloaded" });
  await page.waitForTimeout(1500);
  await page.locator(".v10-tree-dashboard", { hasText: "Marketplace" }).first().click({ force: true });
  await page.waitForSelector(".nsmx", { timeout: 20_000 });
  ...
  await page.waitForSelector(".nsmx .drawer", { timeout: 60_000 });
  ...
  await page.waitForSelector("#nsm-kname", { timeout: 900_000 });   // lane tab-open: minutes, honestly
  ...
  const card = page.locator(".nsmx .mcard .name", { hasText: /7b/i });
  await card.waitFor({ timeout: 60_000 });
  step("J5", "catalog shows BOTH sellers (okf 14B + Hermes 7B) - multi-provider discovery", true, {});
  ...
  await page.waitForSelector(".nsmx .chip--checking", { timeout: 60_000 });
  await shot("07-playground-streaming.png");
  // ✓ lands only after the gateway's final recount event
  await page.waitForSelector(".nsmx .bubble--model .chip--verified", { timeout: 600_000 });
  step("J7", "lane completion (honest non-stream); Verified ✓ landed after recount", true, {});
  ...
  // ═══ 4b. metered query — through the gateway (no UI surface; API step,
  // recorded in the journey log, honest about its path) ═══
  {
    const tok = readFileSync(join(BUYER_HOME, "auth.token"), "utf8").split("\n").map((l) => l.trim()).filter((l) => l && !l.startsWith("#"))[0];
    const qr = await fetch("http://127.0.0.1:9500/marketplace/gateway/v1/query", {
      method: "POST", headers: { authorization: `Bearer ${tok}`, "content-type": "application/json" },
      body: JSON.stringify({ sparql: "SELECT ?s ?p ?o WHERE { ?s ?p ?o } LIMIT 3" }),
      signal: AbortSignal.timeout(600_000),
    }).then((r) => r.json()).catch((e) => ({ error: String(e.message) }));
    const qLeg = qr?.nsm?.leg;
    step("J7b", "metered query over the lane: recounted + countersigned",
      !!qLeg && qr?.nsm?.decision === "countersigned",
      { legId: qLeg?.legId, cost: qLeg?.pricing?.costMicroTrac, error: qr?.error ?? qr?.body ?? undefined });
  }
  ...
  await page.waitForFunction(() => document.body.innerText.includes("Included in close"), { timeout: 600_000 });   // close rides the lane
