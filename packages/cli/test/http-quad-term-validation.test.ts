...
describe('validateWritableQuads', () => {
  it.each([
    ...
    ['an empty-path IRI such as a:', quad({ subject: 'a:', object: '<a:>', graph: 'a:' })],
  ...
  it.each([
    ...
    ['a subject with a malformed percent escape', { subject: 'https://example.org/%zz' }, 'subject'],
    ['a predicate with a caret', { predicate: 'https://schema.org/na^me' }, 'predicate'],
    ['a predicate with a malformed authority', { predicate: 'http://user@@example.org/p' }, 'predicate'],
    ...
    ['an object IRI with a non-numeric port', { object: 'http://example.org:bad/' }, 'object'],
    ...
    ['a literal with a malformed datatype IRI', { object: '"42"^^<https://example.org/%zz>' }, 'object'],
    [
    ...
    ['a graph with a malformed percent escape', { graph: 'https://example.org/%zz' }, 'graph'],
