...
import { DkgHomeFiles, type DkgConfigFileKey } from '../../src/config.js';
...
for (let i = 0; i < Number(count); i += 1) {
  const key = `${writer}-${i}`;
  await files.updateConfigFile([key as DkgConfigFileKey], (config) => {
    (config as Record<string, unknown>)[key] = i;
  });
}
