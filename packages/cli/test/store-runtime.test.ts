...
import { resolveDaemonStoreBootPlan } from '../src/daemon/store-runtime.js';
...
describe('resolveDaemonStoreBootPlan', () => {
  ...
  it('gates an oxigraph-server cutover when legacy store.nq exists without a backend marker', async () => {
    ...
    expect(plan.abortMessage).toContain('DKG_ACCEPT_STORE_RESET=1');
  });
  ...
  it('does not repeatedly gate a cutover already recorded as oxigraph-server', async () => {
    ...
    expect(plan.abortMessage).toBeUndefined();
  ...
  it('migrates an explicit worker config only after acknowledging its legacy store', async () => {
    ...
    expect(blocked.abortMessage).toContain('legacy store.nq from the old worker backend');
    expect(blocked.validateOperatorConfigFirst).toBe(false);

    const acknowledged = resolveDaemonStoreBootPlan({ config, dataDir, acceptStoreReset: true });
    expect(acknowledged.abortMessage).toBeUndefined();
    expect(acknowledged.validateOperatorConfigFirst).toBe(false);
