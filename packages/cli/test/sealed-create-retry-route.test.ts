...
const isCreateFrame = (event: { event: string; data: any }, contextGraphId: string) =>
  event.event === 'memory_graph_changed'
  && event.data?.contextGraphId === contextGraphId
  && event.data?.operation === 'assertion_created';

async function expectNoRetryActivity(): Promise<void> {
  // SSE delivery is ordered. A successful create in a different context graph
  // is a barrier proving every frame from the preceding retry has arrived.
  const barrierCg = `${CG}-barrier-${sseBarrier++}`;
  expect((await postJson(daemon, '/api/context-graph/create', { id: barrierCg, name: barrierCg })).status)
    .toBeLessThan(300);
  expect((await postJson(daemon, '/api/knowledge-assets', { contextGraphId: barrierCg, name: 'barrier' })).status)
    .toBe(201);
  await stream.waitFor((event) => isCreateFrame(event, barrierCg), 8_000);
  expect(stream.events.some((event) => isCreateFrame(event, CG))).toBe(false);
  expect(stream.events.some((event) => (
    event.event === 'notification'
    && event.data?.contextGraphId === CG
    && event.data?.type === 'assertion_activity'
  ))).toBe(false);
}
...
describe('sealed create retry over real HTTP with an isolated mock chain', () => {
  ...
  it('keeps a metadata-only get-or-create request harmless', async () => {
    ...
    expect(retried.body).toMatchObject({ alreadyExists: true, status: 'wm-sealed' });
    expect((await descriptor()).body).toEqual(before.body);
    await expectNoRetryActivity();
  });
