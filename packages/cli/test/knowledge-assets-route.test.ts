...
describe('GitHub-shaped /api/knowledge-assets routes (OT-RFC-43 §10.5)', () => {
  ...
  it('POST .../:name/wm/pull-from resolves DID-form context graph ids before dispatch', async () => {
    ...
    const ctx = ctxFor(
      'POST',
      '/api/knowledge-assets/f/wm/pull-from',
      { contextGraphId: 'did:dkg:context-graph:canonical-cg', layer: 'vm' },
      agent,
    );
    await handleKnowledgeAssetsRoutes(ctx);
    expect(status(ctx)).toBe(200);
    expect(agent.assertion.pullFrom).toHaveBeenCalledWith(
      'canonical-cg',
      'f',
      'vm',
      { subGraphName: undefined, onConflict: 'reject' },
    );
  });
  ...
  it('POST .../:name/wm/pull-from maps an empty source to 409', async () => {
    const agent = makeAssertionAgent({
      assertion: {
        pullFrom: vi.fn(async () => {
          throw Object.assign(new Error('source has no quads'), { code: 'PULL_FROM_EMPTY_SOURCE' });
        }),
      },
    });
    const ctx = ctxFor('POST', '/api/knowledge-assets/f/wm/pull-from', { contextGraphId: 'cg', layer: 'swm' }, agent);
