...
export interface ChainIndexReadCheckpoint {
  readonly id: number;
  readonly method: ChainIndexReadMethod;
  readonly decodedRows: number;
  readonly totalRows: number;
}

export interface ChainIndexReadWorkerHandlerOptions {
  dbPath: string;
  postMessage: (response: ChainIndexReadResponse) => void;
  checkpoint: (progress: ChainIndexReadCheckpoint) => Promise<void>;
}

/** Yield to the worker message loop so queued reads and cancellations can run. */
export async function yieldChainIndexReadWorker(): Promise<void> {
  await yieldTurn();
}

/** Own the read-only database and request state independently of the transport. */
export function createChainIndexReadWorkerHandler({
  dbPath, postMessage, checkpoint,
}: ChainIndexReadWorkerHandlerOptions): {
  handle: (message: ChainIndexReadMessage) => void;
  close: () => void;
} {
  const db = new Database(dbPath, { readonly: true, fileMustExist: true, timeout: 50 });
        ...
        result = await evaluateKnowledgeAssetSnapshot(
          createKnowledgeAssetReadSnapshot(plan, rows, ownWriteHash), registry, {
            ...
            yieldBetweenBatches: async (progress) => {
              await checkpoint({ id: request.id, method: request.method, ...progress });
              assertActive();
            },
          },
        );
  ...
  function handle(message: ChainIndexReadMessage): void {
