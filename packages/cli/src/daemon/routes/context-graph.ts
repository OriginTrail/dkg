  ...
  if (
    ...
    void (async () => {
      ...
      try {
        ...
        // A round stopped by local scheduler pressure did not finish. Surface it
        // as its own retryable status instead of a successful "done", so the
        // caller retries rather than trusting an incomplete catch-up. Readiness
        // patches above stay valid: they are per-peer clean-completion proofs,
        // and a deferred plane never contributes one.
