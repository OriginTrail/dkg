...
import { writeConfigFile, writeConfigSettingsTransaction } from './config-file.js';
...
export class DkgHomeFiles {
  ...
  async saveConfig(config: DkgConfig): Promise<void> {
    await writeConfigFile(this.configPath, () => serializedConfig(config));
  }

  async saveConfigSettingsTransaction(config: DkgConfig, activate: () => undefined): Promise<void> {
    await writeConfigSettingsTransaction(this.configPath, serializedConfig(config), activate);
  }
...
export async function saveConfigSettingsTransaction(config: DkgConfig, activate: () => undefined): Promise<void> {
  await new DkgHomeFiles().saveConfigSettingsTransaction(config, activate);
}
