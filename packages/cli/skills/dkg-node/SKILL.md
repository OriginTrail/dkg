...
| Method | Route | Purpose |
|---|---|---|
| `POST` | `/api/knowledge-assets/{name}/vm/publish-async` | Enqueue VM publish for a named KA already shared to SWM. Body: `{ contextGraphId, options? }`; `options.publisherNodeIdentityIdOverride: "0"` forces no-attribution. Returns `202 { jobId, status: "accepted" }`. Re-submitting the same request resumes the EXISTING job for that KA; `503 { code: "LIFT_JOB_PENDING_CHAIN_PROOF", retryable: true, existingJobId }` means that job failed after it may have submitted a transaction and is held. `retryable` is **true**: chain recovery re-checks that job on every recovery tick and releases it by itself once the chain can say what happened to the transaction, so re-submitting converges without an operator. It is not a deadline, though — while the chain stays silent (the transaction is still in the mempool, or the node cannot reach an endpoint) the hold does **not** expire and you keep getting the 503. If you do not want to wait, check that transaction yourself and clear that job with `POST /api/publisher/clear-job`. Do **not** re-share or publish under a new name to work around it (that is how you double-publish). |
