...
| Method | Route | Purpose |
|---|---|---|
| `POST` | `/api/knowledge-assets/{name}/vm/publish-async` | Enqueue VM publish for a named KA already shared to SWM. Body: `{ contextGraphId, options? }`; `options.publisherNodeIdentityIdOverride: "0"` forces no-attribution. Returns `202 { jobId, status: "accepted" }`. Re-submitting the same request resumes the EXISTING job for that KA; `503 { code: "LIFT_JOB_PENDING_CHAIN_PROOF", retryable: false, existingJobId }` means that job failed after it may have submitted a transaction and is held. `retryable` is **false** because no lane in this build clears the hold on its own — re-submitting on a timer will loop forever. Someone has to end it: check that transaction and clear that job with `POST /api/publisher/clear-job`, or wait for chain recovery to establish its fate. Do **not** re-share or publish under a new name to work around it (that is how you double-publish). |
