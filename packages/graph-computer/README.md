...
3. If the Program calls other Programs, use **Add child Program** to select their approved operations. These must be existing approved operations on this node. Approval pins their binding digests.
...
5. Select the operation graph, allowed caller addresses and execution limits. The operation IRI is generated automatically and the current approval is checked. Review the permission summary and changes, then select **Approve Program** or **Replace approval**. Use **Check approval** to refresh the review. Approval requires the graph owner or node operator; the executor must hold the selected execution identity. An update uses the reviewed revision and reports conflicts without silently overwriting them. Approval compiles the source and displays compilation errors.
...
The editor assigns an operation IRI automatically for a new approval. Review the readable permission summary and the changes from the current revision, then explicitly approve. Graph and operation identifiers and the exact permission diff remain available under **Advanced**. Existing approvals are checked on the node; a draft never restores authority.

Edits autosave in this browser, separately for each node origin, selected agent, source graph and Program. Reopen the same editor to recover source, tool requests, execution settings and arguments. **Discard draft** removes the browser copy without deleting saved versions. Storage failures are shown and unsaved changes retain a close warning. Browser drafts are local, not encrypted, and are not shared with other devices.

New TypeScript executions include a durable call trace with tool/child call order, durations, bounded result previews and failures. The editor displays it alongside the final output, including failures returned by `GraphComputerError.trace`. A child call links to its own execution. These are host call locations, not source-mapped TypeScript line numbers. Traces are retained in the executor's runtime journal and returned again on a retry of the same invocation; retrying a failed TypeScript execution does not repeat effects.

Intermediate results can reveal more than a Program's final output, so detailed traces are returned only to the operation's executor agent after a live authorization check. A caller with only invocation permission receives the final output, without private trace results. Result previews are capped at 4 KiB per call and 64 KiB across the trace; truncated previews and interrupted calls are labelled. Older executions do not gain traces retroactively.
