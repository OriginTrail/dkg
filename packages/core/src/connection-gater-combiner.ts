 ...
 *
 * - `deny*` hooks: denied when ANY fragment denies. Fragments are consulted in
 *   the given order and the first denial short-circuits, so a later policy
 *   neither runs nor logs for a connection an earlier one already refused.
 * - `filterMultiaddrForPeer`: kept only when EVERY fragment keeps the address.
 *
 * A hook no fragment provides stays undefined, which keeps libp2p's own
 * default for it (allow, or store every address). Combined hooks do not depend
 * on `this`: libp2p hands `filterMultiaddrForPeer` to the peer store unbound.
 */
export function combineConnectionGaters(
  ...
  const hookNames = new Set(present.flatMap((fragment) => Object.keys(fragment) as ConnectionGaterHookName[]));
  ...
  for (const name of hookNames) {
    const hooks = present.flatMap((fragment): AnyGaterHook[] => {
      ...
      return hook ? [(...args) => hook.apply(fragment, args)] : [];
    });
    if (hooks.length === 0) continue;
    combined[name] = name === 'filterMultiaddrForPeer'
