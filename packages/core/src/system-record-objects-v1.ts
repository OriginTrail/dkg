...
} from './crypto/workspace-encryption.js';
...
const AGENT_PROFILE_IRI_OBJECT_PREDICATES_V1 = new Set<string>([
  RDF_TYPE,
  ...Object.values(AGENT_PROFILE_LINK_PREDICATES_V1),
  `${SKILL}skill`,
  `${SKILL}pricing`,
...
const AGENT_PROFILE_PUBLIC_ENCRYPTION_KEY_V1 = `${DKG}publicEncryptionKey`;
const AGENT_PROFILE_ALLOWED_TYPE_OBJECTS_V1: Readonly<
...
export function assertAgentProfileProjectionSchemaV1(
  ...
  for (const [index, quad] of quads.entries()) {
    ...
    if (quad.graph !== '') {
      throw new Error('profile projections must be graphless');
    }
    seenSubjects.add(quad.subject);
    if (AGENT_PROFILE_IRI_OBJECT_PREDICATES_V1.has(quad.predicate) === objectIsLiteral) {
      throw new Error('profile projection predicate has an invalid object term kind');
    }
    if (quad.predicate === RDF_TYPE
      && !AGENT_PROFILE_ALLOWED_TYPE_OBJECTS_V1[subjectKind].has(quad.object)) {
    ...
    if (quad.subject === rootSubject) {
      ...
      if (quad.predicate === AGENT_PROFILE_PUBLIC_ENCRYPTION_KEY_V1) {
