 ...
 * still runs.
 *
 * This is deliberately a small concurrency primitive. Transaction labels,
 * timing, logging and lane-health policy belong to SignerTxSerializer.
 */
export class KeyedSerializer {
  private readonly tails = new Map<string, Promise<void>>();

  /** Run `fn` after every previously-submitted operation for `key` settles. */
  run<T>(key: string, fn: () => Promise<T>): Promise<T> {
    const prev = this.tails.get(key) ?? Promise.resolve();
    // A failed predecessor must not skip or wedge the successor.
