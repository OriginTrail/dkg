...
export class ConvictionMethods extends EVMChainAdapterBase implements ConvictionReader {
  ...
  protected override async isConvictionFundedAgent(
    address: string,
   ...
   *   - `profile` / `identity` / `identityStorage` = the identity-key management
   *     write/read surfaces used by the PCA wallet page. These addresses are
  ...
  async getPublishingConvictionContracts(): Promise<PcaContracts> {
    ...
    return {
      ...
      identityStorage: ethers.getAddress(await identityStorage.getAddress()),
