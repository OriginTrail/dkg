  ...
  async createOnChainContextGraph(params: CreateOnChainContextGraphParams): Promise<CreateOnChainContextGraphResult> {
    ...
    const receipt = await (async () => {
      try {
        ...
        // #1340: read the deposit through the RPC-failover facade, not a bare
        // call on the signer's primary-bound contract handle. A broken primary
        // otherwise throws here → is swallowed to 0n → the TRAC approve + retry
        // below never run, defeating failover for a new CG's first publish.
        // `readContract` fails over on transport errors (429/5xx/timeout) and
        // rethrows a decoded revert unchanged; the catch → 0n now fires only
