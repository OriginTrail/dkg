...
describe('V10 Publishing Conviction NFT — chain-adapter lifecycle', () => {
  ...
  it('getIdentityWalletContracts returns the independent identity bootstrap', async () => {
    const reader = createEVMAdapter(HARDHAT_KEYS.CORE_OP);
    const { hubAddress } = getSharedContext();
    const hub = new ethers.Contract(
      hubAddress,
    ...
    const contracts = await reader.getIdentityWalletContracts();
    expect(contracts).toEqual(expect.objectContaining({
      profile: ethers.getAddress(await hub.getContractAddress('Profile')),
      identity: ethers.getAddress(await hub.getContractAddress('Identity')),
      storage: ethers.getAddress(await hub.getContractAddress('IdentityStorage')),
