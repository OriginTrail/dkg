...
import { deterministicId, deterministicSeed } from '../support/fixtures.js';
...
describe('reconciliation symbols', () => {
  it('applies and cancels signed IDs with checksum validation', () => {
    ...
    symbol.count = 2n;
    expect(detectPureSymbol(symbol, seed)).toBeNull();
    expect(detectPureSymbol(emptySymbol(0), seed)).toBeNull();
    const falsePure = emptySymbol(0);
    applyId(falsePure, id, idChecksum(seed, deterministicId('wrong-checksum-id')), 1n);
    expect(detectPureSymbol(falsePure, seed)).toBeNull();
  });
  ...
  it('rejects malformed and overflowing symbols', () => {
    ...
    expect(() => applyId(emptySymbol(0), new Uint8Array(31) as never, idChecksum(seed, id), 1n)).toThrow();
...
describe('rateless encoder', () => {
  it('is insertion-order independent and emits contiguous windows', () => {
    ...
    const first = new RatelessIbltEncoder({
      ids,
      reconciliationSeed: seed,
      algorithm: PAPER_BASELINE_V0.algorithm
    });
    const second = new RatelessIbltEncoder({
      ids: [...ids].reverse(),
      reconciliationSeed: seed,
      algorithm: PAPER_BASELINE_V0.algorithm
    });
  ...
  it('rejects duplicate IDs', () => {
    expect(() => new RatelessIbltEncoder({
      ids: [id, id],
      reconciliationSeed: seed,
      algorithm: PAPER_BASELINE_V0.algorithm
    })).toThrow('duplicate');
  ...
  it('exposes window size and rejects an out-of-order local application', () => {
    const window = new CodingWindow(seed, PAPER_BASELINE_V0.algorithm.mapping);
