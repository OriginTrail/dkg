...
describe('byte primitives and domain-separated hashing', () => {
  ...
  it('binds seed, role ordering, checksum, and mapping seed', () => {
    const requester = deterministicId('requester');
    const provider = deterministicId('provider');
    ...
    expect(() => deriveReconciliationSeed(new Uint8Array(31), provider, nonce)).toThrow();
    expect(() => deriveReconciliationSeed(requester, new Uint8Array(31), nonce)).toThrow();
    expect(() => deriveReconciliationSeed(requester, provider, new Uint8Array(31))).toThrow();
    expect(() => idChecksum(new Uint8Array(31), id)).toThrow();
    expect(() => idChecksum(seed, new Uint8Array(31))).toThrow();
    expect(() => idMappingSeed(new Uint8Array(31), id)).toThrow();
    expect(() => idMappingSeed(seed, new Uint8Array(31))).toThrow();
  });
});
...
describe('candidate profile and mapping schedule', () => {
  it('generates a deterministic increasing membership sequence', () => {
    ...
    const indices = Array.from({ length: 20 }, () => nextMappingIndex(first, PAPER_BASELINE_V0.mapping));
    expect(indices).toEqual(Array.from({ length: 20 }, () => nextMappingIndex(second, PAPER_BASELINE_V0.mapping)));
  ...
  it('rejects invalid cursor, index, and profile values', () => {
    ...
    expect(() => nextMappingIndex(unsafe, PAPER_BASELINE_V0.mapping)).toThrow('safe integer');

    const invalidProfiles = [
      { ...PAPER_BASELINE_V0, mapping: { ...PAPER_BASELINE_V0.mapping, multiplier: 0n } },
      { ...PAPER_BASELINE_V0, mapping: { ...PAPER_BASELINE_V0.mapping, multiplier: 2n } },
      { ...PAPER_BASELINE_V0, mapping: { ...PAPER_BASELINE_V0.mapping, indexOffset: 0 } },
      { ...PAPER_BASELINE_V0, mapping: { ...PAPER_BASELINE_V0.mapping, inverseSqrtNumerator: Number.NaN } },
    ...
    for (const profile of invalidProfiles) expect(() => validateCandidateProfile(profile)).toThrow();
  });
});
