...
export class SymbolWireError extends Error {
  constructor(readonly code: SymbolWireErrorCode, message: string) {
    super(message);
...
class Reader {
  ...
  byte(): number {
    if (this.remaining < 1) throw new SymbolWireError('MALFORMED_CBOR', 'truncated CBOR value');
    return this.bytes[this.#offset++];
  }

  exact(length: number): Uint8Array {
    if (this.remaining < length) throw new SymbolWireError('MALFORMED_CBOR', 'truncated CBOR byte string');
  ...
  unsigned(additional: number): bigint {
    ...
    if (length === undefined) throw new SymbolWireError('MALFORMED_CBOR', 'indefinite or reserved CBOR integer');
    ...
    if (value < minimum) throw new SymbolWireError('NON_CANONICAL_CBOR', 'CBOR integer is not shortest-form');
    return value;
  }
  ...
  integer(): bigint {
    ...
    throw new SymbolWireError('MALFORMED_CBOR', 'expected a CBOR integer');
  }

  bytes32(): Uint8Array {
    const head = this.byte();
    if (head >>> 5 !== 2) throw new SymbolWireError('MALFORMED_CBOR', 'expected a CBOR byte string');
    const length = this.unsigned(head & 0x1f);
    if (length !== 32n) throw new SymbolWireError('MALFORMED_CBOR', 'symbol byte strings must be exactly 32 bytes');
...
export function decodeReconciliationSymbolV1(bytes: Uint8Array): ReconciliationSymbolV1 {
  ...
  if (reader.byte() !== 0x84) throw new SymbolWireError('MALFORMED_CBOR', 'symbol must be a four-item CBOR array');
