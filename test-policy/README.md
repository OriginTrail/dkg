...
The trusted controller's `ci-delta.mjs` owns the planner lane IDs and each lane's aggregate job (`PRIMARY_LANE_JOBS`). `ci-lanes.mjs` derives the executable candidate-side topology from that map: coverage package membership and shard counts per lane. The build job emits its matrices; every coverage job consumes those matrices and invokes `run-vitest-lanes.mjs --job JOB --row ROW`. That runner derives package membership, shard arguments and concurrency from the same model. Agent subdivisions use the existing agent shard policy, without a second total. One definition change updates workflow dispatch, runner denominators and expected receipts; generic boundary validation rejects literal matrices or bypassing the runner. The supporting job runs up to three packages concurrently with two workers per package, waits for every result, and then executes the required Python and demo steps even if a Vitest lane failed. Each package retains its own JUnit and coverage receipt. This concurrency limit is not a measured CI speedup claim.
