...
CI collects coverage during existing Vitest executions. Individual shards temporarily disable aggregate thresholds; the required `coverage-results` job verifies exact package/shard membership, revision, source/config/lockfile fingerprints and executed-test evidence, merges Istanbul hit maps, then enforces source scope and floors. It does not average shard percentages. The runner parses JUnit XML and requires at least one executed, non-skipped test and no errors before writing a version-2 receipt; aggregation rechecks that execution count. Missing, stale, duplicate, empty, entirely skipped or failed shards cannot produce a passing coverage verdict. Two real RDF Utils shards were checked locally against the unsharded report and produced identical totals.
...
The trusted controller owns planner lane IDs and the aggregate job mapping (`PRIMARY_LANE_JOBS` in `scripts/lib/ci-delta.mjs`, which must stay loadable from its sparse checkout). `ci-lanes.mjs` derives the executable candidate-side topology from that map and is the source for coverage package membership and shard counts. The build job emits its matrices; every coverage job consumes those matrices and invokes `run-vitest-lanes.mjs --job JOB --row ROW`. That runner derives package membership, shard arguments and concurrency from the same model. Agent subdivisions use the existing agent shard policy, without a second total. One definition change updates workflow dispatch, runner denominators and expected receipts; generic boundary validation rejects literal matrices or bypassing the runner. The supporting job runs up to three packages concurrently with two workers per package, waits for every result, and then executes the required Python and demo steps even if a Vitest lane failed. Each package retains its own JUnit and coverage receipt. This concurrency limit is not a measured CI speedup claim.

The disabled-test scanner separates pure AST analysis, Git/file discovery and CLI dispatch. A single per-file AST and import-resolution model detects disabled tests, focused tests and invalid exception metadata; compatibility wrappers retain the existing D1/D2 fingerprint API. Its former embedded self-tests run in the tooling suite. Imported test aliases, namespace imports, computed skip members and chained APIs cannot bypass the debt ledger; locally shadowed imports are ignored. The original 72-finding ledger is unchanged; one reviewed discovery exclusion from canary PR #2473 is registered separately with its required EVM execution route and source revision.

Shared KA request fixtures and owned native-server startup live under `scripts/testing`, with an explicit ESM boundary. Backend conformance no longer imports a publisher-private test module or a package-private port helper. The publisher helper-type gate still checks the shared request contract, and each consumer declares specific shared fixture inputs in its package Turbo configuration. Coverage fingerprints read those same declarations. A gate checks real fixture imports against the declarations, and mutation tests compare actual Turbo task hashes: KA changes affect Publisher, snapshot changes affect Agent/CLI, property-option changes affect Core/Agent/Publisher, and conformance-only Oxigraph changes affect none of these package tasks. Build invalidation is limited to Publisher's compiled helper contract.
...
After integration with canary `f2cacb9c6`, all 336 tooling tests passed and inventory accounted for 1,741 test files. The native Hardhat regression now runs two complete setups concurrently through the real configuration and deployment helper, checks distinct RPC URLs and an untouched repository manifest, and verifies that tearing down one setup leaves the other and a foreign listener alive. Successful deployments also require a matching Hub record in their own temporary manifest before cleanup; a zero-exit child without that record is rejected.

The next review round integrated canary `d05e6d3c1`. Agent unit shards now apply the same production-only coverage policy as the primary config; shard 6 passed 435 tests and emitted a version-2 receipt covering 299 production files with no test/helper paths. Local checks passed 351 script tests, the 1,758-file inventory, all 25 build tasks, workflow lint and targeted XML/topology/scanner regressions. Core passed 1,872 tests with two workers and unchanged coverage floors; its unrestricted overlapping run retained three timing failures. The additional real-runtime FD smoke passed with the other 26 relay cases. The canary publish-JSON-LD merge passed all 38 cases. A real 25-test Hardhat Identity run created a new report directory and passed the Mocha validator; a regression also exercises the exact Hardhat-installed Mocha reporter. These local results do not substitute for the new GitHub candidate runs.

The final follow-up replaces duplicated Hardhat/daemon subprocess state with `scripts/testing/owned-process.mjs`: readiness races child failure and timeout, diagnostics drain through `close`, and teardown is idempotent and awaits close after SIGTERM/SIGKILL escalation. Service probes retain their own stdout/RPC or private-port-file/HTTP evidence. Real concurrent chains and daemons passed; all 34 dedicated EVM cases passed with parsed, executed-test JUnit evidence. The gossip case stops and drains receiver reconciliation, observes the real finalization handler, and checks the local query before restarting the poller. The tooling suite passed 356 tests, inventory covered 1,759 files, strict fixture types and lint passed. GitHub's initial dynamic-matrix injection finding was reproduced with pinned zizmor 1.7.0 and repaired by passing the row through a quoted environment variable; the same offline audit then reported no findings. Fresh online GitHub checks are required on the pushed head.

The remaining focused review changes extract the complete relay-capacity constants, types, validation and derivation into a dependency-free module, keeping the existing package/deep exports compatible and giving FD headroom its own named policy. Core passed 1,873 tests and retained 87.03% full-source line coverage; all 54 changed executable lines met the 90% gate. Teardown regressions now prove the stopped chain RPC and daemon HTTP endpoint are unreachable while the other child and unrelated listener remain alive. The two-node Markdown journey passed with a shared readiness helper using the canonical shared-memory topic and the node's wire-ID mapping. All 36 focused capacity/reservation cases, Core compilation, repository lint and the 1,759-file inventory also passed.

A final repeat passed all 1,873 Core cases but exposed a line-coverage variation to 86.99%, just below the unchanged 87% floor. Only `node.ts` varied between reports. Three deterministic tests now exercise the public relay-stats API through a stubbed libp2p boundary: forwarding-direction counts, current reservation details and updates, malformed-entry/connection handling, and unavailable stats after shutdown. The resulting full run passed all 1,876 tests with 87.26% lines, all 54 changed executable lines covered, and no relaxed floors. Repository lint and the updated 1,760-file inventory passed. The earlier floor failure remains recorded separately.
