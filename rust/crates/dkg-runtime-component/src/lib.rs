...
use bindings::exports::origintrail::semantic_runtime::runtime::{
    AdapterVersion, AdmissionDiagnostic, AdmittedPlan, Completion, Diagnostic, EffectRequest,
    EffectResult, Execution, ExecutionReceipt, Guest, GuestExecution, LogicalAgent, PlanValue,
    ResourceBounds, SourcePosition, SourceSpan, Step,
};
...
impl GuestExecution for ExecutionState {
    async fn advance(&self, result: Option<EffectResult>) -> Result<Step, Diagnostic> {
        let input = match result {
            None => PlanApplyInput::Continue,
            Some(result) => PlanApplyInput::EffectResult {
                effect_id: result.effect_id,
                result: result.outcome.map_err(|failure| failure.code),
            },
        };
        ...
        decode_step(&success_payload(&response, message_type::APPLY_PLAN)?)
    }
...
fn decode_step(payload: &[u8]) -> Result<Step, Diagnostic> {
    let mut decoder = Decoder::new(payload);
    let length = decoder
        .array()
        .map_err(decode_error)?
    ...
    let step = match (tag, length) {
        (0, 6) => {
            ...
            Step::EffectRequested(effect)
        }
        (1, 3) => Step::Completed(Completion {
