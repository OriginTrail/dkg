...
function own(file, lane, command, cadence) {
...
try {
  ...
  const tracked = [...new Set(execFileSync('git', ['ls-files', '--cached', '--others', '--exclude-standard', '-z'], { cwd: root, encoding: 'utf8', maxBuffer: 32 * 1024 * 1024 }).split('\0'))].filter((file) => /\.(?:test|spec|test-d)\.[cm]?[jt]sx?$/.test(file) || /^packages\/adapter-hermes\/pytests\/test_.*\.py$/.test(file) || file.endsWith('.test.yaml')).filter((file) => fs.existsSync(path.join(root, file)));
  for (const route of registrations) {
    ...
    const matches = tracked.filter((file) => path.posix.matchesGlob(file, route.pattern));
    if (!matches.length) throw new Error(`stale test route: ${route.pattern}`);
    for (const file of matches) if (!owners.has(file)) own(file, route.lane, route.command === 'resolve' ? secondaryCommand(file) : route.command, route.cadence);
  }
