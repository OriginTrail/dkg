...
/**
 * Canonical manifest of the runtime-config assets the CLI copies in from the
 * repo root, as package-relative paths (what must appear inside the tarball).
 * The single source of truth shared by the pack-time copy (below) and the
 * `release:verify-pack` preflight (scripts/release-packages.mjs), so a new
 * overlay flows to both automatically instead of being tracked in two lists.
 * `build-info.json` is NOT here — it is generated by `release:build-info`, not
 * copied, so the verifier adds it as an explicit release-only extension.
 */
export function cliRuntimeAssetRelPaths({ rootDir = DEFAULT_ROOT_DIR } = {}) {
  const overlays = enumerateNetworkOverlays(path.join(rootDir, 'network')) ?? [];
  return ['project.json', ...overlays.map((file) => `network/${file}`)];
}
...
export function copyCliRuntimeAssets({ rootDir = DEFAULT_ROOT_DIR } = {}) {
  ...
  const networkJsonFiles = enumerateNetworkOverlays(sourceNetworkDir);
  if (networkJsonFiles === null) {
