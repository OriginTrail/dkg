...
The dashboard signs in with this token automatically only when it is opened on the node host itself (`http://127.0.0.1:9200/ui` or `http://localhost:9200/ui`). Opened from another machine or under another host name, it asks for the token instead; an SSH tunnel to the API port keeps the automatic sign-in. A reverse proxy that adds forwarding headers (`Forwarded`, `X-Forwarded-*`, `X-Real-IP`, `Via`) is treated as remote. One on the node host that forwards with a loopback `Host` and adds none of them (e.g. nginx's default `proxy_set_header Host $proxy_host`) still looks like a local caller, so keep authentication in front of any proxy that exposes the dashboard beyond the node host.
