...
describe('DKG v10 Publish/Query Lifecycle', function () {
  ...
  before(async function () {
    ...
    try {
      ...
      console.log(`Node "${status.name}" is running (peerId: ${status.peerId}, role: ${status.nodeRole}, identity: ${status.identityId})`);
    } catch (err) {
      console.error(`Node not reachable at ${DKG_API_URL}: ${err.message}`);
      throw err;
    }

    console.log(`--- Ensuring context graph "${DKG_CONTEXT_GRAPH_ID}" exists (registered on-chain)`);
    try {
      const cg = await httpCreateContextGraph(DKG_CONTEXT_GRAPH_ID, 'V10 Publish Test CG', 'Automated publish/query test context graph');
      console.log(`Context graph "${DKG_CONTEXT_GRAPH_ID}" ready${cg.onChainId ? ` (on-chain ${cg.onChainId})` : ''}`);
    } catch (err) {
      console.log(`Context graph setup: ${err.message} (may already exist, continuing)`);
    }
  });
  ...
  it('should publish and query KAs sequentially in batches', async function () {
    ...
    console.log(`Context graph: ${DKG_CONTEXT_GRAPH_ID}`);
    ...
    const processKnowledgeAsset = async (kaNumber) => {
      ...
      try {
        ...
        const result = await Promise.race([
          httpPublish(DKG_CONTEXT_GRAPH_ID, quads),
        ...
        if (result.status !== 'confirmed') {
          throw new Error(`Publish returned "${result.status}" instead of "confirmed" (kaId: ${result.kaId}, httpStatus: ${result.httpStatus})${result.error ? ` — ${result.error}` : ''}`);
        }
      ...
      try {
        ...
        const aqResult = await Promise.race([
          httpQuery(assetSparql, DKG_CONTEXT_GRAPH_ID),
          new Promise((_, reject) =>
  ...
  ?s schema:name ?name ;
     schema:description ?description .
        ...
        const queryResult = await Promise.race([
          httpQuery(generalSparql, DKG_CONTEXT_GRAPH_ID),
          new Promise((_, reject) =>
    ...
    // ---- Run batches (same pattern as V9 / V8) ----
    ...
    // ---- Write summary JSON (compatible with the V8/V9 DB schema) ----
    const summary = {
    ...
    // ---- Write error JSON (compatible with the V8/V9 DB schema) ----
    const errorData = {
