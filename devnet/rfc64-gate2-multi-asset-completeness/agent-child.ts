...
export interface Gate2AgentSpawnSpec {
...
export interface Gate2AgentChildOptions {
  ...
  readonly role: 'author' | 'receiver';
  readonly spawn: Gate2AgentSpawnSpec;
}
...
interface PendingEvent {
  ...
  readonly resolve: (event: Gate2AgentEvent) => void;
  readonly timer: NodeJS.Timeout;
}

export class Gate2AgentChild {
  readonly child: ChildProcessWithoutNullStreams;
  readonly tracked: TrackedChildProcess;
  readonly #eventTimeoutMs: number;
  readonly #events: Gate2AgentEvent[] = [];
  readonly #pending = new Set<PendingEvent>();
  #stderr = '';
  #stdout = '';
  #lineBuffer = '';

  constructor(readonly options: Gate2AgentChildOptions) {
  ...
  waitFor(expectedEvent: string, requestId: string | null = null): Promise<Gate2AgentEvent> {
    ...
    return new Promise<Gate2AgentEvent>((resolveEvent, rejectEvent) => {
  ...
  async request(
    ...
    if (Buffer.byteLength(line) > MAX_EVENT_LINE_BYTES) {
      throw new Error('Gate 2 adapter command exceeds the 1 MiB process-protocol bound');
    }
  ...
  private consumeStdout(chunk: string): void {
    ...
    for (const line of lines) {
      if (!line.startsWith(GATE2_AGENT_EVENT_PREFIX)) continue;
      let event: Gate2AgentEvent;
      try {
        event = JSON.parse(line.slice(GATE2_AGENT_EVENT_PREFIX.length)) as Gate2AgentEvent;
      } catch (error) {
        this.rejectAll(new Error(`invalid Gate 2 adapter event JSON: ${String(error)}`));
        continue;
      }
