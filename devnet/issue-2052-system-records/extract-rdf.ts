...
ORDER BY ?s ?p ?o`.trim(),
  ...
  const source: CharacterizationSourceV1 = {
    ...
    provenance: {
      ...
      redactionPolicy: 'V1: wallet roots, peer IDs, and referenced key IDs replaced by deterministic fixture-local ordinal aliases; record/root aliases are mutually bound; only exact allowlisted profile subject shapes and predicates retained; unknown or underived nested subjects rejected before serialization; literal values omitted but UTF-8 byte lengths retained; no tokens, keys, multiaddrs, names, raw identifiers, or hashes of identifiers stored',
