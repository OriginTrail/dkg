...
ORDER BY ?s ?p ?o`.trim(),
  ...
  const source: CharacterizationSourceV1 = {
    ...
    provenance: {
      ...
      redactionPolicy: 'V1: wallet roots, peer IDs, and referenced key IDs replaced by deterministic fixture-local ordinal aliases that preserve source lexicographic rank; record/root aliases are mutually bound; only exact allowlisted profile subject shapes and predicates retained; unknown or underived nested subjects rejected before serialization; literal values omitted but UTF-8 byte lengths retained; aliases and per-record shape/byte counts may be correlated with the public agents graph; no tokens, keys, multiaddrs, names, raw identifiers, or hashes of identifiers stored',
...
function buildProfiles(
  ...
  for (const root of activeRoots) {
    ...
    const redactSubject = (subject: string) => {
      ...
      const kind = classifyProfileSubjectShapeV1(root, subject);
      const redacted = kind === 'x25519'
        ? redactProfileSubjectV1(root, redactedRoot, subject, ++x25519Ordinal)
        : redactProfileSubjectV1(root, redactedRoot, subject);
      subjectMap.set(subject, redacted);
      return redacted;
    };
    const linkedSubjects = new Set<string>();
    const derivedSourceSubjects = new Set<string>();
    const referencedSourceX25519 = new Set<string>();
    for (const row of rootRows) {
      ...
      try {
        ...
        derivedSourceSubjects.add(keyId);
      } catch {
    ...
    for (const row of rootRows) {
      ...
      const subjectKind = classifyProfileSubjectShapeV1(root, subject.value);
      const linkedKind = object.type === 'uri'
        ? classifyProfileSubjectShapeV1(root, object.value)
        : null;
      if (subjectKind === 'x25519') referencedSourceX25519.add(subject.value);
      if (object.type === 'uri' && linkedKind === 'x25519') {
        referencedSourceX25519.add(object.value);
    ...
    if ([...referencedSourceX25519].some((subject) => !derivedSourceSubjects.has(subject))) {
      throw new Error('profile references an x25519 subject not derived from published key material');
    }
    const retainedDerivedSubjects = [...referencedSourceX25519]
      .map(redactSubject)
      .sort();
    profiles.push({
      ...
      derivedSubjects: retainedDerivedSubjects,
      quads: quadShapes,
...
function parseSystemSyncInput(value: unknown): SanitizedSystemSyncInputV1 {
  ...
  for (const sourceUrl of input.sourceUrls) assertEvidenceSourceUrlV1(sourceUrl);
...
async function select(endpoint: URL, query: string): Promise<SparqlRow[]> {
  ...
  return decodeSparqlResults(await response.json());
}

export function decodeSparqlResults(value: unknown): SparqlRow[] {
  if (!isRecord(value) || !isRecord(value.results) || !Array.isArray(value.results.bindings)) {
    throw new TypeError('SPARQL response has no bindings');
  }
  return value.results.bindings.map((binding) => {
    if (!isRecord(binding)) throw new TypeError('SPARQL binding must be an object');
    return Object.fromEntries(
      Object.entries(binding).map(([key, term]) => [key, decodeSparqlTerm(term)]),
    );
  });
}

function decodeSparqlTerm(value: unknown): SparqlTerm {
  if (
    !isRecord(value)
    || (value.type !== 'uri' && value.type !== 'literal' && value.type !== 'bnode')
    || typeof value.value !== 'string'
    || (value.datatype !== undefined && typeof value.datatype !== 'string')
    || (value['xml:lang'] !== undefined && typeof value['xml:lang'] !== 'string')
  ) {
    throw new TypeError('SPARQL term is malformed');
  }
  return {
    type: value.type,
    value: value.value,
