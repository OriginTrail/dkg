...
export function isGate1VmChainInventorySelected(
  agent: { isVmReconcileTargetSelected(contextGraphId: string): boolean },
  contextGraphId: string,
): boolean {
  return agent.isVmReconcileTargetSelected(contextGraphId);
}
...
export class Gate1RolloutAdapterFixture {
  ...
  async seedVmSourceSwm(
    ...
    return seedGate1VmSourceSwm(agent, contextGraphId);
  }
}
...
export async function seedGate1VmSourceSwm(
  ...
  return agent.publisher.stageKnowledgeAssetSharedWorkingMemoryV1({
    contextGraphId,
    shareOperationId,
    kaUal: GATE1_KA_UAL,
    assertionVersion: '1',
