...
function parseShareJobId(stdout: string): string {
  const jobId = /^\s*Job ID:\s+(.+)$/m.exec(stdout)?.[1]?.trim();
  if (!jobId) throw new Error(`could not parse share job id from:\n${stdout}`);
  return jobId;
}
...
function writeNtFixture(name: string, value: string): { filePath: string; subject: string } {
  ...
  const subject = `urn:test:pr1410:${name}:${Date.now()}:${++fileCounter}`;
...
beforeAll(async () => {
  ...
  if (!detected) {
    throw new Error(
      'No devnet detected - run ./scripts/devnet.sh start 6 and pnpm run build before this suite.',
    );
  }
  await ensureAllIdentities(detected, 4);
  const node = detected.nodes[1]!;

  const slug = unique('pr1410-ka-cli');
  ...
  const subGraphName = unique('pr1410sg').replace(/[^a-zA-Z0-9-]/g, '');
...
describe('KA lifecycle CLI on devnet', () => {
  it('one-shot create/share reaches sub-graph SWM and async VM publish accepts the named KA', async () => {
    ...
    const body = lastJsonBlock(publish.stdout);
  ...
  it('stepwise lifecycle drains share-async, then pull-from/query/discard works', async () => {
    ...
    const wmBeforeFinalize = await runDkgCli(node, ['ka', 'query', name, '-c', contextGraphId, '--json'], 60_000);
    expectCliOk(wmBeforeFinalize, 'ka query before finalize');
    expect(Number(lastJsonBlock(wmBeforeFinalize.stdout).count ?? 0)).toBeGreaterThan(0);
    ...
    const jobId = parseShareJobId(enqueued.stdout);
    ...
    await waitFor(`share job ${jobId} succeeded`, 180_000, 3_000, async () => {
      ...
      const job = lastJsonBlock(detail.stdout);
      if (job.state === 'failed') {
    ...
    const listed = (lastJsonBlock(jobs.stdout).jobs ?? []) as Array<{ jobId?: string }>;
    ...
    const quads = (lastJsonBlock(wmAfterPull.stdout).quads ?? []) as Array<{ subject?: string }>;
