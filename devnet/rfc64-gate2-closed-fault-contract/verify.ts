...
/**
 * RFC-64 Gate 2 verifier entrypoint (FIXTURES-ONLY).
 *
 * Reads the generated raw evidence, runs the fail-closed contract verifier,
 * atomically writes the verdict, prints VERDICT_SHA256 and the status, and
 * exits non-zero when the fixture does not satisfy the contract.
 */
...
const rawArtifactPath = process.env.DKG_RFC64_GATE2_RAW_ARTIFACT ?? DEFAULT_RAW_ARTIFACT;
const verdictArtifactPath =
  process.env.DKG_RFC64_GATE2_VERDICT_ARTIFACT ?? DEFAULT_VERDICT_ARTIFACT;
...
try {
  ...
  throw new Error('Gate 2 raw evidence is not valid UTF-8 JSON', { cause });
}
if (Buffer.from(stableJson(parsed)).equals(rawBytes) === false) {
  throw new Error('Gate 2 raw evidence is not the exact canonical stable JSON encoding');
}

const verdict = verifyGate2(parsed);
...
process.stdout.write(
  `[rfc64-gate2-verifier] productBoundary=${verdict.productBoundary} gateEvaluation=${verdict.gateEvaluation}\n`,
);
process.stdout.write(`[rfc64-gate2-verifier] verdict artifact: ${verdictArtifactPath}\n`);
process.stdout.write(
  '[rfc64-gate2-verifier] fixtures-only: "contract-satisfied" means the fixture matches the contract, NOT that the product passed Gate 2\n',
);

if (verdict.status !== 'contract-satisfied') {
  for (const violation of verdict.violations) {
    process.stderr.write(`[rfc64-gate2-verifier] violation ${violation.id}: ${violation.detail}\n`);
  }
  process.exitCode = 1;
}
...
function readStableRegularFile(path: string): Buffer {
  ...
  if (parentTopology.isSymbolicLink() || !parentTopology.isDirectory()) {
    throw new Error(`Gate 2 artifact parent must be a non-symlink directory: ${dirname(path)}`);
  }
  const beforeTopology = lstatSync(path);
  if (beforeTopology.isSymbolicLink() || !beforeTopology.isFile()) {
    throw new Error(`Gate 2 artifact must be a non-symlink regular file: ${path}`);
  }
  if (process.platform !== 'win32' && (beforeTopology.mode & 0o777) !== 0o600) {
    throw new Error(`Gate 2 artifact must have mode 0600: ${path}`);
  }
  ...
  if (
    ...
    throw new Error(`Gate 2 artifact topology changed while it was read: ${path}`);
  }
  return bytes;
}
